...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt canonical OpenAI-style messages to native Ollama /api/chat.

    The app carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        ...
        h.setdefault("X-OpenRouter-Title", wl.APP_NAME)
    if provider == "copilot":
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip app-only metadata before sending messages to providers.
