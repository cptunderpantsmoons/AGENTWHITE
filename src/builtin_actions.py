...
async def action_run_script(owner: str, script: str = "", host: str = "", **kwargs) -> Tuple[str, bool]:
    ...
    target_host = (host or os.getenv("WL_SCRIPT_HOST", os.getenv("ODYSSEUS_SCRIPT_HOST", "localhost"))).strip()
                    ...
                    for uid_b in uids:
                        ...
                        try:
                            ...
                            if not raw:
                                continue
                            msg = _email_mod.message_from_bytes(raw)
                            # Skip app-generated reminders so the scanner
                            ...
                            _ody_origin = (msg.get(wl.header("Origin")) or "").strip().lower()
                            _ody_kind = (msg.get(wl.header("Kind")) or "").strip().lower()
                            ...
                            if (_ody_origin == wl.MAIL_ORIGIN or _ody_kind == "reminder"
...
BUILTIN_ACTION_INFO = {
    ...
    "run_script": "Run a script locally or on WL_SCRIPT_HOST (or legacy ODYSSEUS_SCRIPT_HOST)",
