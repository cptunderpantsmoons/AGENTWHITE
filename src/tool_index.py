...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "operator_run_task": "Delegate a task to the Operator-Use multi-agent orchestrator. The operator can use browser automation, desktop automation, or reasoning depending on the task. Good for complex multi-step workflows: 'check my email and calendar and tell me if I have any conflicts', 'go to example.com, extract the pricing table, and paste it into a document'.",
    ...
    "web_use_run": "Run a natural-language browser automation task using Web-Use. The agent controls a real Chromium browser via CDP. Good for: 'open SharePoint, navigate to Finance folder, download Q4.xlsx', 'go to outlook.com and check unread emails', 'sign into the admin portal and create a new user'.",
    "windows_use_run": "Run a natural-language Windows desktop automation task using Windows-Use. Controls Windows desktop applications via UI Automation. Good for: 'open Outlook desktop app and reply to the latest email from Jane', 'open File Explorer and navigate to Downloads', 'open Notepad and type Hello World'. Only available on Windows hosts.",
}
