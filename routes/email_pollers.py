                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in the app (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in app" button
                                    import html as _h
    ...
    try:
        ...
        for r in rows:
            ...
            try:
                ...
                outer[wl.header("Origin")] = wl.MAIL_ORIGIN
                outer[wl.header("Kind")] = re.sub(r"[^A-Za-z0-9_.-]", "-", odysseus_kind or "scheduled")[:64]
                outer[wl.header("Ref")] = sid
                if r[6]:
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `WL_INPROCESS_POLLERS` (or legacy `ODYSSEUS_INPROCESS_POLLERS`).
    Set to `0`/`false`/`no`/`off` to disable the asyncio tasks so a cron /
    systemd-timer setup driving `odysseus-mail poll-scheduled` is the sole
    external driver."""
    import os
    raw = os.environ.get("WL_INPROCESS_POLLERS", os.environ.get("ODYSSEUS_INPROCESS_POLLERS", "1")).strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when polling is disabled — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled; "
