...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When the app runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends the app's own interpreter directory to PATH. If the app itself is
...
def _user_shell_path_bootstrap() -> list[str]:
    return [
        'APP_USER_SHELL="${SHELL:-}"',
        'if [ -n "$APP_USER_SHELL" ] && [ -x "$APP_USER_SHELL" ]; then',
        '  APP_USER_PATH="$("$APP_USER_SHELL" -ic \'printf "__APP_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__APP_PATH__//p\' | tail -n 1 || true)"',
        '  if [ -n "$APP_USER_PATH" ]; then export PATH="$APP_USER_PATH:$PATH"; fi',
        'fi',
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by the app.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$APP_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $APP_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        ...
        runner_lines.append('  exit "$APP_PREFLIGHT_EXIT"')
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for app-managed `pip install --user`
    # installs, but then report the actual CLI path so external runtimes are clear.
    runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
    runner_lines.append('APP_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"')
    runner_lines.append('if [ -z "$APP_VLLM_BIN" ]; then')
    runner_lines.append('  echo "ERROR: vLLM is not installed."')
    runner_lines.append('  APP_PREFLIGHT_EXIT=127')
    runner_lines.append('else')
    runner_lines.append('  echo "[app] vLLM CLI: $APP_VLLM_BIN"')
    runner_lines.append('  APP_VLLM_VERSION="$("$APP_VLLM_BIN" --version 2>&1 | head -n 1 || true)"')
    runner_lines.append('  if [ -n "$APP_VLLM_VERSION" ]; then echo "[app] vLLM version: $APP_VLLM_VERSION"; fi')
    runner_lines.append('fi')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('APP_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $APP_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $APP_CMD_EXIT ==="')
        ...
        runner_lines.append('echo ""; echo "=== Process exited with code $APP_CMD_EXIT ==="')
        runner_lines.append('exit "$APP_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[app] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('        echo "[app] CUDA nvcc + cudart found — building llama-server with CUDA (GPU) support..."')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[app] WARNING: nvcc found but CUDA runtime (libcudart.so) is not visible — building llama-server for CPU only."')
    runner_lines.append('        echo "[app]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('        echo "[app]   Ensure libcudart is installed (e.g. cuda-runtime package) and visible via ldconfig or CUDA_HOME."')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    else')
    runner_lines.append('      echo "[app] WARNING: no HIP/CUDA toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[app]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[app]   Install ROCm for AMD GPUs or vLLM/CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
...
def _llama_cpp_rebuild_cmd() -> str:
    ...
    return (
        ...
        'echo "[app] Cleared the cached llama.cpp build. '
